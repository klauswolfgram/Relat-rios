...
Private lFilDeAte := Iif(Empty(mv_par23), .F., .T.)
Private lFiliais  := If(mv_par25==1,.T.,.F.)
Private aLisFil	  := {} 
Private cFilDe    := mv_par22
...
If lFiliais
	...
	nConsFil :=1
ElseIf lFilDeAte
	aLisFil :=FilRange(cFilDe,CFilAte)
Else 
	aLisFil:={{.T.,cFilAnt}}
	nConsFil := 1
EndIf

If Empty(aLisFil) //Nao foi selcionado Filial corrente no tratamento de filiais.
	RETURN
EndIf

aDadosApur:=ResumeF3("IC",dDtIni,dDtFim,cNrLivro,lQbrAliquota,lQbrCFO,2,@lAbortPrint,1,cFilDe,;
					 cFilAte,@aResEntr,@aResSaid,AllTrim(aReturn[7]),,,,,,lImpCrdSt,;
					 lMv_UFSt,lCrdEst,NIL,NIL,NIL,NIL,NIL,NIL,NIL,"MATR941",;
					 mv_par24,aLisFil)
...
If lFiliais .or. lFilDeAte //Executa o laço para localizar o arquivo correto no caso de ter mais de uma filial selecionada
	For nFil := 1 to Len(aLisFil)
		If aLisFil[nFil, 1] // Considera somente filial selecionada
			If nApuracao==1.or.nApuracao==2.or.!lConcilia	
				AADD(aArqApur,NmArqApur("IC",nAno,nMes,nApuracao,nPerApurado,cNrLivro,,aLisFil[nFil][2]))
			Else
				Do Case
					Case nApuracao==3
						nMesIni:=nMes
				...
				For i:=nMesIni to nMesFim		
					AADD(aArqApur,NmArqApur("IC",nAno,i,1,0,cNrLivro,,aLisFil[nFil][2]))
					AADD(aArqApur,NmArqApur("IC",nAno,i,1,1,cNrLivro,,aLisFil[nFil][2]))
					AADD(aArqApur,NmArqApur("IC",nAno,i,1,2,cNrLivro,,aLisFil[nFil][2]))
					AADD(aArqApur,NmArqApur("IC",nAno,i,1,3,cNrLivro,,aLisFil[nFil][2]))
				Next i
			Endif
		EndIf
	Next
	
Else //Executa a localização do arquivo sem passar filial pois vai ser a cFilAnt
	If nApuracao==1.or.nApuracao==2.or.!lConcilia	
		...
		While(!FT_FEof())
			...
			If cLinha <> Chr(10) .And. cLinha <> Chr(0)
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ		
					...
					Case cCampo=='TIT'
						If nTamLinha >84
							cTit := STR0079 + Substr(cTexto,1,TamSX3("F2_DOC")[1]) + Space(03) + STR0092 + Substr(cTexto,TamSX3("F2_DOC")[1]+1,18) + Space(03) + STR0093 + alltrim(Substr(cTexto,35,35))
						Else
							cTit := STR0079 + Substr(cTexto,1,9) + Space(03) + STR0092 + Substr(cTexto,10,18) + Space(03) + STR0093 + alltrim(Substr(cTexto,35,35))
						Endif
						AAdd( aTit, cTit )
					Case cCampo=='GNR'
						If nTamLinha >90
					...
					Case cCampo=='FOM' //Fomentar - GO
						...
						If Substr(Alltrim(cTexto),1,3) == "084" .And. nAno < 2023
							AADD(aObs,"--------------------------------------------------------------------------------------------------------------------------------")
							AADD(aObs,"")						
						ElseIf Substr(Alltrim(cTexto),1,3) == "119" .And. nAno >= 2023
							AADD(aObs,"--------------------------------------------------------------------------------------------------------------------------------")
							AADD(aObs,"")
						Endif					
					...
					Case cCampo=="OUT" .And. Substr(clinha,5,3) == "012"
					   ...
					   For nXF := 1 To Len(aObs)
							If "ICMS Complementar (Diferencial de Aliquotas) :  " $ aObs[nXF]
								aObs[nXF] := "ICMS Complementar (Diferencial de Aliquotas) :  "+ Transform((Val(StrTran(StrTran( Alltrim(substr(aObs[nXF],47,(Len(aObs[nXF])))),".",""),",","."))),X3PICTURE("F3_ICMSCOM") )																
							EndIf	 
						Next(nXF) 
						AADD(aObs,Trim(cTexto)+" : "+Transform(nValor,"@E 999,999,999,999.99"))
				EndCase
...
Return
/*/{Protheus.doc} FilRange

Função que fará carregamento das Filiais selecionadas de ate "MV_PAR22 e MV_PAR33"

//O aFilsSelec{} recebe as Filiais
...
Static Function FilRange(cFilDe,cFilAte)
...
Local cFilBack := cFilAnt
	...
	CFILANT := PADR(ALLTRIM(cFilDe), LEN(CFILANT))

	While !SM0->(Eof()) .and. FWGrpCompany()+FWCodFil() <= cEmpAnt+cFilAte

	    cFilAnt := PadR(Alltrim(SM0->M0_CODFIL),Len(cFilAnt))

		AAdd(aFilsSelec,{.T., PadR(Alltrim(SM0->M0_CODFIL),Len(cFilAnt)), SM0->M0_FILIAL, SM0->M0_CGC, SM0->M0_INSC, SM0->M0_INSCM})
		SM0->(DBSkip())
	EndDo
	RestArea(aAreaSM0)	
...
Local aInfoFilDe  := {}
Local aAreaSM0    := {}
Local lFilDif	  := !(alltrim(cFilDe) $ alltrim(cFilant))
Local nPosFil	  := 0
Local nX          := 0
Local cFilSelect  := ""
Local lFilDif2    := .f.

If !lFiliais .and. lFilDeAte .and. lFilDif
	aAreaSM0 := SM0->(GetArea())

	dbSelectArea("SM0")
	dbSeek(FWGrpCompany()+cFilDe,.T.)

	Aadd(aInfoFilDe, {SM0->M0_NOMECOM , SM0->M0_INSC,  TRANSFORM(SM0->M0_CGC,"@R! NN.NNN.NNN/NNNN-99")})
...
If lFiliais
 	For nX := 1 to Len(aLisFil)
		If aLisFil[nX, 1]
			...
			aAreaSM0 := SM0->(GetArea())
			dbSelectArea("SM0")
			dbSeek(FWGrpCompany()+aLisFil[nX, 2],.T.)

			cFilSelect := SM0->M0_NOMECOM

			RestArea(aAreaSM0)
			lFilDif2 := Alltrim(aLisFil[nX, 2]) $ Alltrim(cFilAnt)
...
cNome  := Iif(lFiliais .and.  !lFilDif2, cFilSelect,         Iif(lFilDeAte .and. lFilDif .and. !lFilDif2, aInfoFilDe[1,1], SM0->M0_NOMECOM))
cInscr := Iif(lFiliais .and.  !lFilDif2, aLisFil[nPosFil,5], Iif(lFilDeAte .and. lFilDif .and. !lFilDif2, aInfoFilDe[1,2], InscrEst()))
cCGC   := Iif(lFiliais .and.  !lFilDif2, TRANSFORM(aLisFil[nPosFil,4],"@R! NN.NNN.NNN/NNNN-99"), Iif(lFilDeAte .and. lFilDif .and. !lFilDif2, aInfoFilDe[1,3], TRANSFORM(SM0->M0_CGC,"@R! NN.NNN.NNN/NNNN-99")))
