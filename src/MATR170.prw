...
±±³Sintaxe   ³ U_MATR170(ExpC1,ExpN1,ExpN2)                           	  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
User Function Matr170(cAlias,nReg,nOpcx)
...
oFornec2 := TRSection():New(oSection1,STR0093,{"SA2"},/*Ordem*/) //"Fornecedor (Parte 2)" 
		 ...
		 oCell := TRCell():New(oFornec2,"A2_CGC" ,"SA2",,"@X",(10+Len(AllTrim(RetTitle("A2_CGC")))+TamSX3("A2_CGC")[1]),,{|| AllTrim(RetTitle("A2_CGC"))+': '+If(cPaisLoc<>"BRA",Transform(SA2->A2_CGC,PesqPict("SA2","A2_CGC")),Transform(SA2->A2_CGC,PicPesFJ(If(Len(AllTrim(SA2->A2_CGC))<14,"F","J")))) } )
...
If lAuto
	dbSelectArea("SF1")
	dbGoto(nReg)
	cWhere += " AND F1_FILIAL = '" + SF1->F1_FILIAL + "'"
	cWhere += " AND F1_DOC = '" + SF1->F1_DOC + "'"
	cWhere += " AND F1_SERIE = '" + SF1->F1_SERIE + "'"
	cWhere += " AND F1_FORNECE = '" + SF1->F1_FORNECE + "'"
	cWhere += " AND F1_LOJA = '" + SF1->F1_LOJA + "'"
	cWhere += " AND F1_TIPO = '" + SF1->F1_TIPO + "'"
EndIf
...
cCampos += "D1_TOTAL,D1_IPI,D1_IPI,D1_PICM,D1_CONTA,D1_CC,D1_CF,D1_CUSTO,D1_RATEIO,D1_ITEMCTA,D1_CLVL,SD1.D1_NUMSEQ,D1_EMISSAO,D1_COD," 
...
While !oReport:Cancel() .And. !(cAliasSF1)->(Eof()) .And. ;
    ...
    If nCT == 0
		aFornece := {{cFornece,cLoja,PadR(MVNOTAFIS,Len(SE2->E2_TIPO))},;
					...
					{PadR(GetMv('MV_FORINSS')      ,Len(SE2->E2_FORNECE)),PadR(Replicate('0',Len(SE2->E2_LOJA)),Len(SE2->E2_LOJA)),PadR(MVINSS,Len(SE2->E2_TIPO))},;
					{PadR(GetMv('MV_MUNIC')        ,Len(SE2->E2_FORNECE)),PadR(Replicate('0',Len(SE2->E2_LOJA)),Len(SE2->E2_LOJA)),PadR(MVISS ,Len(SE2->E2_TIPO))},;
					{PadR(GetMv('MV_FORSEST')      ,Len(SE2->E2_FORNECE)),PadR(Replicate('0',Len(SE2->E2_LOJA)),Len(SE2->E2_LOJA)),PadR("SES" ,Len(SE2->E2_TIPO))} }    
