...
@description
Relatório de Ocupação de Veículos por Tipo de Veículo,
Mostra o aproveitamento da capacidade dos veículos por por Tipo de Veículo pela carga embarcada
-------------------------------------------------------------*/
User Function GFER153()
	Local oReport
	Local aArea := GetArea()
...
Static Function ReportDef()
	...
	//| "                                                                        | "
	//+------------------------------------------------------------------------+
	oReport := TReport():New("GFER153","Relatório de Ocupação de Veículo por Tipo de Veículo","GFER153", {|oReport| ReportPrint(oReport)},"Emite relatório de Ocupação de Veículos por Tipo de Veículo conforme os parâmetros informados."/*Descricao do relatório*/)
	...
	EndIf
	oSection := TRSection():New(oReport,"Ocupação de Veículos",{"(cAliasOc)"},{"Ocupação de Veículos por Tipo de Veículo"}/*{Array com as ordens do relatório}*/,/*Campos do SX3*/,/*Campos do SIX*/)
	...
	TRCell():New(oSection,"(cAliasOc)->GV3_CDTPVC","(cAliasOc)","Tipo de Veículo",GetSx3Inf("GV3_CDTPVC")[3],/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection,"(cAliasOc)->GV3_DSTPVC","(cAliasOc)","Descrição",GetSx3Inf("GV3_DSTPVC")[3],/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
...
Static Function CarregaDados()
	...
	Local cFltrGWN := "", cFltrGU3 := "", cFltrGV3 := "" 
	CriaTabTemp()
	...
	*/ 
	...
	cFltrGV3 += " AND GV3_CDTPVC >='" + MV_PAR10 + "' AND GV3_CDTPVC <='" + MV_PAR11 + "' "
	...
	cQuery += "			AND GWN_CDTPVC = GV3_CDTPVC" + cFltrGWN + cFltrGU3 
	cQuery += "		) GWN_TOTROM,"
	...
	cQuery += "			INNER JOIN " + RetSQLName("GU3") + ""
	cQuery += "			ON GU3_CDEMIT = GWN_CDTRP" 
	cQuery += "			WHERE " + RetSQLName("GW1") + ".D_E_L_E_T_ = ' ' AND " + RetSQLName("GWN") + ".D_E_L_E_T_ = ' ' AND " + RetSQLName("GU3") + ".D_E_L_E_T_ = ' '"
	cQuery += "			AND GWN_CDTPVC = GV3_CDTPVC" + cFltrGWN + cFltrGU3
	cQuery += "		) GW1_QTVOL"
	cQuery += "	FROM	(SELECT"	
	cQuery += "				GWN_CDTPVC," 
	...
	cQuery += "				ON GU3_CDEMIT = GWN_CDTRP"  
	cQuery += "				WHERE	GW1.D_E_L_E_T_ = ' ' AND GW8.D_E_L_E_T_ = ' ' AND GWN.D_E_L_E_T_ = ' ' AND GU3.D_E_L_E_T_ = ' '" + cFltrGWN + cFltrGU3
	cQuery += "				GROUP BY GWN_CDTPVC"
	cQuery += "			) ITENS INNER JOIN"
	cQuery += "			(SELECT"
	cQuery += "				GWN_CDTPVC ,"
	...
	Else	
		...
		cQuery += "				LEFT JOIN " + RetSQLName("GU3") + " ON GU3_CDEMIT = GWN_CDTRP"  
	...
	cQuery += "				GROUP BY GWN_CDTPVC"
	cQuery += "			) SOMAPESO" 
	cQuery += "			ON SOMAPESO.GWN_CDTPVC = ITENS.GWN_CDTPVC"  
	cQuery += "			INNER JOIN " + RetSQLName("GV3") + " GV3" 
	cQuery += "			ON GV3_CDTPVC = ITENS.GWN_CDTPVC" 
	cQuery += "			WHERE GV3.D_E_L_E_T_ = ' '" + cFltrGV3
...
A funcao estatica CriaTabTemp Cria a struct dos dados do relatório e cria as tabelas temporárias, privadas em U_GFER153() 
...
Static Function CriaTabTemp()
   ...
   aStruOpTot := { {"GWN_TOTROM","N",9                        ,0                      },; // QUANTIDADE ROMANEIOS
                   {"PESOCAPAC" ,"N",TamSX3("GWI_VLFRET")[1]+3,TamSX3("GWI_VLFRET")[2]},; // CAPACIDADE PESO VEÍCULOS OU VEÍCULO
                   {"GW8_PESOR" ,"N",TamSX3("GW8_PESOR" )[1]+3,TamSX3("GW8_PESOR" )[2]},; // PESO EMBARCADO
                   {"LOTACPESO" ,"N",TamSX3("GWI_VLFRET")[1]+3,2                      },; // LOTAÇÃO DO PESO
                   {"PESOBRUMED","N",TamSX3("GWI_VLFRET")[1]+3,2                      },; // PESO BRUTO MÉDIO
                   {"VOLUCAPAC" ,"N",TamSX3("GWI_VLFRET")[1]+3,TamSX3("GWI_VLFRET")[2]},; // CAPACIDADE VOLUME VEÍCULOS OU VEÍCULO
                   {"GW8_VOLUME","N",TamSX3("GW8_VOLUME")[1]+3,TamSX3("GW8_VOLUME")[2]},; // VOLUME EMBARCADO
                   {"LOTACVOLU" ,"N",TamSX3("GWI_VLFRET")[1]+3,2                      },; // LOTAÇÃO DO PESO
                   {"VOLUMED"   ,"N",TamSX3("GWI_VLFRET")[1]+3,2                      },; // VOLUME MÉDIO
                   {"GW1_QTVOL" ,"N",TamSX3("GW1_QTVOL" )[1]+3,TamSX3("GW1_QTVOL" )[2]},;
                   {"QTVOLMED"  ,"N",TamSX3("GW1_QTVOL" )[1]+3,2                      } }
