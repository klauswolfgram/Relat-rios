...
If cPaisLoc == "BRA"
	...
	While !oReport:Cancel() .And. !(cAliasSD1)->(Eof())
		...
		If dbSeek(xFilEnt(xFilial("SC7"))+(cAliasSD1)->D1_COD+(cAliasSD1)->D1_PEDIDO+(cAliasSD1)->D1_ITEMPC,.F.)
			nValorSC7 := IIf(Empty(SC7->C7_REAJUST),xMoeda(SC7->C7_PRECO,SC7->C7_MOEDA,1,(cAliasSD1)->D1_EMISSAO,TamSX3("C7_PRECO")[2],SC7->C7_TXMOEDA),Formula(SC7->C7_REAJUST))
