...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function CTBR010R4( cMoeda )
Local oReport
...
TRCell():New( oSection1, "CT1_DESC01"	,"CT1" ,STR0021/*Titulo*/,/*Picture*/,aTamDesc01[1]+5,/*lPixel*/,{||If( ! Empty( mv_par06 ) , &("CT1->CT1_DESC" + mv_par06 ) , CT1->CT1_DESC01)})
...
If Upper( FunName() ) == 'CTBR015'
	...
	oSection11  := TRSection():New( oSection1, "Tipificação", {"CT1"}, aOrdem, .F., .F. ) //"Conta Contábil"
...
cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_FILIAL = '" + xFilial("CT1") + "'" )

IF ! Empty( mv_par01 ) .AND. ( mv_par01 <> NIL )
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_CONTA  >= '" + mv_par01 + "'" )
Endif

If ! Empty( mv_par02 ) .And. ( mv_par02 <> NIL )
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_CONTA  <= '" + mv_par02 + "'" )
Endif


IF mv_par03 == 2
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_NCUSTO = 0" )
EndIF

If mv_par05 == 1
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_CLASSE = '2'" )

Elseif mv_par05 == 2
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_CLASSE = '1'" )
EndIf

If mv_par07 == 2
	cFilter :=  U_AddcFilter( cFilter , "CT1->CT1_BLOQ = '2'" )
EndIf
...
oSection1:OnPrintLine( {|| U_CT10onprint( oReport ) })
...
User Function CT10onprint( oReport )
...
*/
User Function AddcFilter( cStrFilter , cStrAdd )
	LOCAL cCond := " "
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function Ctbr010R3(wnRel,dDataRef,cMoeda)
...
Local lAltom := FWIsInCallStack("UTSTARTRPT") 
...
±±³Descri‡…o ³ Programa de Impressao do Plano de Contas              	  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe   ³ U_Ctbr010(lEnd,wnRel,cString)                                ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Retorno   ³ Nenhum                                                     ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ CTBR010                                                    ³±±
...
Local nMasc			:= 1
Local nMaxCol		:= 220
Local cContaAnt		:= '' // verificação da conta anterior, usado para o saldo da linha
Local cClasseAnt	:= '' // verificação da conta anterior, usado para o saldo da linha
Local lImprime 		:= .T. 
Local cAlias		:= "CT1"
Local aCampos
...
If nOrdem == 1
	...
	If SuperGetMv("MV_CTASUP") = "S"
		...
		aCampos := { { "CONTA"    , "C", Len(CT1->CT1_CONTA), 0 },;
		...
		__oTempTable := FWTemporaryTable():New( "cArqTmpI" )  
		__oTempTable:SetFields(aCampos) 
		__oTempTable:AddIndex("1", {"CONTA"})
		__oTempTable:AddIndex("2", {"ORDEMPRN"})
		__oTempTable:AddIndex("3", {"SUPERIOR"})		
...
//Exclui tabela temporaria do banco de dados
If __oTempTable <> Nil
	__oTempTable:Delete()
	__oTempTable := Nil
Endif
