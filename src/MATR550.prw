...
If cPaisLoc == "BRA"
	...
	TRCell():New(oCabec,"CNOTA"		,/*Tabela*/,cTitD2Doc		             , cPicD2Doc ,cTamD2Doc         	        ,/*lPixel*/,{|| cNota })
	TRCell():New(oCabec,"CSERIEVIEW",/*Tabela*/,SerieNfId("SD2",7,"D2_SERIE"), cPicD2Ser ,SerieNfId("SD2",6,"D2_SERIE")	,/*lPixel*/,{|| cSerieView })
...
oReport:Section(2):Cell("CNOTA"):SetBlock({|| cNota})
oReport:Section(2):Cell("CSERIEVIEW"):SetBlock({|| cSerieVIEW})
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//Cria coluna de Num do Doc e Série somente em modo planilha e tabela      ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If lImpPlan
	oReport:Section(2):Cell("CNOTA"):Show()	
	oReport:Section(2):Cell("CSERIEVIEW"):Show()	
Else
	oReport:Section(2):Cell("CNOTA"):Disable()	
	oReport:Section(2):Cell("CSERIEVIEW"):Disable()	
EndIf
...
While !oReport:Cancel() .And. !(cAliasQry)->(Eof())
	...
	dbSelectArea(cAliasQry)
	If ( cTipoNF == 'C' .Or. (nAcd1 + nAcD4 + nAcD5) > 0) .And. ( dEmisAnt != (cAliasQry)->F2_EMISSAO .Or. (cAliasQry)->(Eof()))
